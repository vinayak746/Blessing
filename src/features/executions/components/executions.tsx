            ...
            {statusFilters.map((filter) => {
              ...
              return (
                <Button
                  ...
                  size="sm"
                  className={`h-7 rounded-md px-2.5 text-xs border transition-[background-color,border-color,color,box-shadow] duration-200 ${
                    isActive
    ...
    <EntityItem
      ...
      actions={
        <Button
          size="sm"
          variant="outline"
          className="h-8 px-2.5 text-xs border-border/70 bg-background/75 dark:bg-card hover:bg-accent/70 dark:hover:bg-accent/80"
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
